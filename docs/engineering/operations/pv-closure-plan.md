...
| 11 | `PV-MA-002` | 原生录制 covered | 真实 native capture 主路径、音频产物和标准门禁覆盖不足 | 将 opt-in ScreenCaptureKit smoke 推进到 release-scope native recording smoke，并定义系统/麦克风缺失时的降级证明 | `platform/native-app/scripts/native-capture-smoke.sh`; app-bundle real capture smoke |
| 12 | `PV-MA-003` | 会话和录制产物登记 covered | stop 产物字段、降级原因和真实 capture 产物链不完整 | 对 `screen_video`/`system_audio`/`microphone_audio`/`mixed_audio` 的 available/missing/degraded 登记做真实 capture smoke 和文件契约覆盖 | native capture smoke; session artifact tests |
