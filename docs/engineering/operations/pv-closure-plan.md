...
| 13 | `PV-MA-009` | 文件化流水线重试和原始媒体保护 covered | VS-MA-21 provider hardening release gate 和 app-bundle same-chain 已补；剩余依赖真实 ScreenCaptureKit/native UI 并发重试、跨机器可重复性和 release bundle | 保留 `release-capture-processing-hardening-smoke.sh` 作为 provider-side 标准 gate；在 release candidate 中补真实 native/release-scope 并发、重试、原始媒体保护和 bundle 证据 | `platform/e2e/release-capture-processing-hardening-smoke.sh`; `./scripts/check.sh`; `./scripts/test-e2e-full-stack.sh`; `./scripts/release-preflight.sh` |
