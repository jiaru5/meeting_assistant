...
| PV-MA-006 | `CAP-MA-006` 标准化处理音频和输入选择，不覆盖原始音频 | `docs/product-spec/05-business-rules-and-calculations.md`, `docs/product-spec/07-data-and-events.md`, `docs/product-spec/09-acceptance-criteria.md` | 文件契约测试、样例媒体处理测试、source selection 单元测试 | `platform/processing-cli/scripts/test.sh`; 未来真实 FFmpeg/兼容 adapter smoke 和 native 处理入口 | `processing-cli` 已实现内部 fixture-compatible normalized audio stage，组件测试覆盖默认选择 `mixed_audio`、缺失 `mixed_audio` 时必须显式选择可用 `system_audio`/`microphone_audio`、无默认音频返回 `artifact_missing`、仅 WAV/PCM 源可登记为 `normalized_audio.wav`、重复执行复用已登记 `normalized_audio` 且不改变原始音频 checksum、显式换源在未定义 replace policy 前返回 `path_conflict`、处理失败写入 `logs/processing.log` 且不登记派生产物；尚未覆盖真实媒体转码 runtime 或 native UI 处理入口 | `partial` |
