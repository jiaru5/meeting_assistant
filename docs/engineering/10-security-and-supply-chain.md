...
`./scripts/supply-chain-check.sh release` 必须在上述组件 SBOM/report 聚合之外，额外校验 release provenance/signing evidence report。默认路径是 `.harness/evidence/release/supply-chain/release-provenance-report.json` 和 `.harness/evidence/release/supply-chain/release-signature-report.json`；CI 或人工 release rehearsal 可通过 `MEETING_ASSISTANT_RELEASE_PROVENANCE_REPORT` 和 `MEETING_ASSISTANT_RELEASE_SIGNATURE_REPORT` 指向实际产物。provenance report 必须声明 manifest 中的 `provenance_target`、`sbom_format`、当前 commit、builder、source repository、`release_provenance_attestation=produced` 和至少一个 `sha256:` artifact digest；signature report 必须声明 manifest 中的 `artifact_signing`、当前 commit、`signing_status=signed`、verifier 和至少一个 signed artifact。缺少这些 report 时 release 阶段必须 fail closed，不能把 validation-only SBOM 当作 SLSA/signing 放行证据。
