...
`./scripts/release-inputs-report.py` 可在 CI 或人工 release rehearsal 已经产出真实 release 物料后，生成 release bundle/provenance/signature 三份输入 report。该脚本要求调用方显式提供签名/公证后的 Release zip、DSSE in-toto SLSA provenance v1 attestation、Sigstore bundle、builder/source repository、签名身份、公证 ticket、OIDC certificate identity/issuer 和 transparency log 信息；脚本只做 archive shape、digest 和 DSSE subject 绑定检查，不负责构建、签名、公证、staple、上传公证或签发 Sigstore/SLSA 物料。它写出的 report 必须继续被 `release-bundle-check.sh` 和 `supply-chain-check.sh release` 独立验证，不能作为 release readiness 的替代证据。
