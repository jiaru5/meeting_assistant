...
`./scripts/supply-chain-check.sh release` 必须在上述组件 SBOM/report 聚合之外，额外校验 release provenance/signing evidence report。默认路径是 `.harness/release-inputs/supply-chain/release-provenance-report.json` 和 `.harness/release-inputs/supply-chain/release-signature-report.json`；CI 或人工 release rehearsal 可通过 `MEETING_ASSISTANT_RELEASE_PROVENANCE_REPORT` 和 `MEETING_ASSISTANT_RELEASE_SIGNATURE_REPORT` 指向实际产物。provenance report 必须声明 manifest 中的 `provenance_target`、`sbom_format`、当前 commit、builder、source repository、`release_provenance_attestation=produced` 和至少一个 `sha256:` artifact digest；signature report 必须声明 manifest 中的 `artifact_signing`、当前 commit、`signing_status=signed`、verifier 和至少一个 signed artifact。缺少这些 report 时 release 阶段必须 fail closed，不能把 validation-only SBOM 当作 SLSA/signing 放行证据。

`./scripts/release-bundle-check.sh` 是 release artifact guard，必须先证明存在签名、公证、staple、digest 和当前 commit 绑定的 `MeetingAssistantNative.app` Release zip archive，且该 bundle 不打包 runtime/model、不自动下载、不包含会议数据。该 gate 必须解压 archive 并对 `MeetingAssistantNative.app` 运行 `codesign --verify --deep --strict`、`codesign -dv`、`xcrun stapler validate` 和 `spctl -a -t exec`；JSON report 只能提供输入索引和期望字段，不能替代制品本体校验。该 gate 与 `supply-chain-check.sh release` 互补：bundle report 证明分发制品本身，provenance/signing report 证明 SLSA/signature 供应链证据；二者不能相互替代。
