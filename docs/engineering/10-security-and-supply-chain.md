...
`./scripts/release-bundle-check.sh` 是 release artifact guard，必须先证明存在签名、公证、staple、digest 和当前 commit 绑定的 `MeetingAssistantNative.app` Release archive，且该 bundle 不打包 runtime/model、不自动下载、不包含会议数据。该 gate 与 `supply-chain-check.sh release` 互补：bundle report 证明分发制品本身，provenance/signing report 证明 SLSA/signature 供应链证据；二者不能相互替代。
