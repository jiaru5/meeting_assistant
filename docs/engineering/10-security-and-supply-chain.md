...
`./scripts/supply-chain-check.sh release` 必须在上述组件 SBOM/report 聚合之外，额外校验 release bundle/provenance/signing/sidecar evidence report。默认路径是 `.harness/release-inputs/bundle/release-bundle-report.json`、`.harness/release-inputs/supply-chain/release-provenance-report.json`、`.harness/release-inputs/supply-chain/release-signature-report.json` 和 `.harness/release-inputs/supply-chain/release-sidecar-report.json`；CI 或人工 release rehearsal 可通过 `MEETING_ASSISTANT_RELEASE_BUNDLE_REPORT`、`MEETING_ASSISTANT_RELEASE_PROVENANCE_REPORT`、`MEETING_ASSISTANT_RELEASE_SIGNATURE_REPORT` 和 `MEETING_ASSISTANT_RELEASE_SIDECAR_REPORT` 指向实际产物。bundle report 必须声明当前 commit、`release_gate=release-bundle`、Release zip archive、`MeetingAssistantNative.app` 和 `sha256:` bundle digest；provenance report 必须声明 manifest 中的 `provenance_target`、`sbom_format`、当前 commit、builder、source repository、`release_provenance_attestation=produced`，artifact 列表必须包含与 bundle report 同一 digest、名称匹配 release zip 或 `.app` 的 artifact，并且必须引用 digest 匹配的 DSSE in-toto SLSA provenance v1 attestation 文件。checker 必须解析 DSSE payload，要求 statement `_type=https://in-toto.io/Statement/v1`、`predicateType=https://slsa.dev/provenance/v1`、包含同一 bundle subject digest，并具备 `buildDefinition.buildType` 和 `runDetails.builder.id`。signature report 必须声明 manifest 中的 `artifact_signing`、当前 commit、`signing_status=signed`、verifier，且同一 bundle digest 的 signed artifact 必须提供 `signature_type=keyless-oidc`、certificate identity、certificate issuer、transparency log 和 digest 匹配的 Sigstore bundle 文件；sidecar report 必须绑定当前 commit，声明 `release_gate=release-sidecar-portability`、`target_scope=all-target-machines`、不打包 runtime/model、不自动下载、不访问外网，并为每个目标机器记录 `.local` 下 runtime、model、mixed-language smoke audio fixture 的 digest/source 以及 `check_dependencies`、`whisper.cpp` smoke 和 no-auto-download 通过状态；每个目标机器还必须引用 digest 匹配的 `release-sidecar-target-smoke` report 文件，且该文件必须绑定当前 commit 和 target id/os/architecture，记录同一 runtime/model/audio digest，并声明未打包 runtime/model、未自动下载、未访问外网和 smoke 全部通过。缺少这些 report、provenance/signature 指向不同 artifact digest、attestation/signature bundle 文件缺失、目标机 smoke report 文件缺失、文件 digest 不匹配或文件语义与目标机 sidecar 不一致时 release 阶段必须 fail closed，不能把 validation-only SBOM、本机 provider smoke、单机 sidecar 证据或不可审计的 SLSA/signing JSON 声明当作发布放行证据。

`./scripts/release-bundle-check.sh` 是 release artifact guard，必须先证明存在签名、公证、staple、digest 和当前 commit 绑定的 `MeetingAssistantNative.app` Release zip archive，且该 bundle 不打包 runtime/model、不自动下载、不包含会议数据。该 gate 必须解压 archive 并对 `MeetingAssistantNative.app` 运行 `codesign --verify --deep --strict`、`codesign -dv`、`xcrun stapler validate` 和 `spctl -a -t exec`；JSON report 只能提供输入索引和期望字段，不能替代制品本体校验。该 gate 与 `supply-chain-check.sh release` 互补：bundle report 证明分发制品本身，supply-chain release gate 会用同一 bundle digest 约束 DSSE/SLSA provenance attestation 和 Sigstore signing bundle，sidecar report 及其每台目标机 smoke report 证明目标机器 runtime/model/audio 取证可重复；三者不能相互替代，也不能互相指向不同 release artifact。

`./scripts/release-bundle-create.py` 是 release artifact production guard。负责签名/公证的 release rehearsal 或 CI job 必须显式提供组织批准的 `Developer ID Application` signing identity 和 notarytool keychain profile；脚本会先运行 `release-credential-check.py`，然后用 Xcode Release build、manual code signing、notarytool submit `--wait`、stapler 和 `ditto --keepParent` 产出 zip，并写出 release bundle report。它默认继续调用 `release-bundle-check.sh`，所以本体签名、公证、staple、Gatekeeper、digest 或当前 commit 绑定失败都会阻断。该入口不得自动下载依赖、runtime、model 或测试音频，也不得生成 SLSA/DSSE attestation、Sigstore bundle 或 signature report；这些仍由独立 supply-chain/release job 生成并由 `release-inputs-report.py`、`supply-chain-check.sh release` 验证。

`./scripts/release-candidate-inputs.py` 是 release rehearsal orchestration guard。它可以串联 `release-bundle-create.py`、`release-inputs-report.py --verify-release-bundle`、`release-bundle-check.sh` 和 `supply-chain-check.sh release`，确保 release archive、DSSE/SLSA attestation、Sigstore bundle、bundle/provenance/signature reports 和 sidecar report 绑定到同一 release candidate 输入集。该入口必须接收真实 attestation 和 Sigstore bundle 文件，不能自行生成、伪造或降级 provenance/signature，也不能替代 all-target sidecar target smoke；任何缺失文件、digest 不匹配、bundle 本体校验失败或 release supply-chain gate 失败都必须保留原始失败。

`./scripts/release-credential-check.py` 是 release 物料产出前置检查，不是 release artifact guard。release rehearsal 或负责签名/公证的 CI job 应先运行该脚本，确认当前环境有 Apple release 工具链和匹配 `Developer ID Application:` 的 codesigning identity；可用 `--identity-pattern` 收紧为组织批准的签名身份，并用 `--report` 保存 `release_gate=release-credential-prereq` 诊断 report。该 report 不包含 secret，也不证明 app 已经签名、公证、staple、上传 notary、生成 SLSA 或签发 Sigstore bundle；它只把“当前环境没有产出 release 物料的最低凭据”提前 fail closed。只验证既有 release 输入的 CI job 可以跳过该 prereq check，继续依赖 `release-bundle-check.sh` 和 `supply-chain-check.sh release` 验证制品本体与 provenance/signature。

`./scripts/release-inputs-report.py` 可在 CI 或人工 release rehearsal 已经产出真实 release 物料后，生成 release bundle/provenance/signature 三份输入 report。该脚本要求调用方显式提供签名/公证后的 Release zip、DSSE in-toto SLSA provenance v1 attestation、Sigstore bundle、builder/source repository、签名身份、公证 ticket、OIDC certificate identity/issuer 和 transparency log 信息；脚本默认做 archive shape、digest 和 DSSE subject 绑定检查。release rehearsal 或 CI 应加 `--verify-release-bundle`，让脚本在写 report 前解压 archive 并对其中 `MeetingAssistantNative.app` 运行 `codesign`、`stapler` 和 `spctl`，提前拒绝 ad-hoc 签名、签名身份不符或未通过公证/Gatekeeper 的物料。该脚本不负责构建、签名、公证、staple、上传公证或签发 Sigstore/SLSA 物料。它写出的 report 必须继续被 `release-bundle-check.sh` 和 `supply-chain-check.sh release` 独立验证，不能作为 release readiness 的替代证据。
