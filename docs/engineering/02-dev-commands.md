...
| `release-candidate-inputs.py` | VS-MA-23 release rehearsal 编排入口：可先调用 `release-bundle-create.py` 产出签名/公证 zip，再用真实 DSSE/SLSA attestation 和 Sigstore bundle 调 `release-inputs-report.py`，最后运行 `release-bundle-check.sh` 与 `supply-chain-check.sh release`；不生成或伪造 provenance/signature |
| `release-bundle-create.py` | 在具备 `Developer ID Application` identity 和 notarytool keychain profile 的 release rehearsal/CI 环境中，构建 Release app、签名、公证、staple、打包 zip 并写出 release bundle report；默认继续调用 `release-bundle-check.sh`，但不生成 DSSE/SLSA 或 Sigstore evidence |
...
| `release-inputs-report.py` | 从已经生成的签名/公证 Release zip、DSSE/SLSA provenance attestation 和 Sigstore bundle 物料化 `.harness/release-inputs/` 下的 bundle/provenance/signature reports；可用 `--verify-release-bundle` 在写 report 前对 archive 内 `.app` 跑 `codesign`、`stapler` 和 `spctl` 提前拦截错误物料；该入口不构建、不签名、不公证，也不绕过后续校验 |
...
`./scripts/release-bundle-create.py` 是 release bundle 产出入口。release rehearsal 或负责 Apple 分发包的 CI job 必须显式提供 `MEETING_ASSISTANT_RELEASE_SIGNING_IDENTITY` 或 `--signing-identity`，以及 `MEETING_ASSISTANT_NOTARYTOOL_PROFILE` 或 `--notary-profile`；脚本会先调用 `release-credential-check.py` 确认本机具备匹配的 Developer ID identity，再用 Xcode 构建 Release app、手动签名、提交 notarytool `--wait`、staple、用 `ditto --keepParent` 产出 zip，并写出 `.harness/release-inputs/bundle/release-bundle-report.json`。默认完成后继续运行 `release-bundle-check.sh`，所以 ad-hoc 签名、notary 非 Accepted、未 staple、Gatekeeper 拒绝或 report/archive digest 不一致仍会 fail closed。该入口不生成 DSSE/SLSA provenance attestation、不调用 Sigstore/cosign、不生成 signature report，也不替代 `release-inputs-report.py`、`supply-chain-check.sh release` 或 `release-preflight.sh`。

`./scripts/release-candidate-inputs.py` 是 VS-MA-23 release rehearsal 的总编排入口。默认顺序为：调用 `release-bundle-create.py` 产出签名/公证/stapled Release zip 和 bundle report；要求调用方显式提供真实 DSSE in-toto SLSA provenance attestation 与 Sigstore bundle；调用 `release-inputs-report.py --verify-release-bundle` 写出 bundle/provenance/signature reports；最后运行 `release-bundle-check.sh` 和 `supply-chain-check.sh release`。如果 release bundle 已由 CI 预先产出，可用 `--skip-bundle-create` 复用既有 archive 和 bundle report，但 provenance/signature 仍必须绑定同一 digest。该入口不创建 SLSA attestation、不签发 Sigstore bundle、不生成 sidecar target evidence，也不把 release inputs 通过外推为 `PV-MA-*` covered；最终放行仍以 `product-validation-check.py release` 和 `release-preflight.sh` 为准。
