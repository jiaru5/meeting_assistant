...
| `supply-chain-check.sh` | 验证依赖/Action 固定，运行组件 SBOM gate，聚合校验组件供应链报告；`release` 模式还必须看到 release bundle report、匹配同一 bundle digest 的 DSSE/SLSA provenance attestation、keyless signing bundle 和目标机器 sidecar repeatability evidence report，否则 fail closed |
