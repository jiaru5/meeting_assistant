...
./scripts/mvp1-experience-study.py --help
./scripts/mvp1-accessibility-walkthrough.py --help
./scripts/mvp1-task-xcresult.py --help
./scripts/mvp1-screenshot-visual-review.py --help
...
| `release-preflight.sh` | 发布候选总入口：release blocker、验证矩阵、open decisions、生成物和全量门禁；默认 `local-direct` 分发模式会安装稳定本机 app 并运行 local-direct functional preflight，显式 `developer-id` 模式才走更重的 legacy release-scope native capture/XCUITest gates |
...
`./scripts/test-e2e-full-stack.sh` 本地默认设置 `MEETING_ASSISTANT_SMOKE_IMAGE=meeting-assistant-smoke:local`，由 `platform/e2e/prepare-smoke-image.sh` 验证或从已缓存允许镜像 tag 出本地 smoke image；脚本仍不得自动 pull。准备脚本先用 `docker image inspect` 探测 target/base image，再用 `docker run --pull=never --network none ... sh -c 'sleep 0'` 作为最终可运行性验证；当 Docker Desktop image-store 短暂让 `inspect` 看不到已缓存镜像但 no-pull run probe 成功时，允许继续使用该本地镜像。`platform/e2e/docker-compose.smoke.yml` 的默认镜像值必须保持 digest-pinned，以满足 release manifest 静态门禁；本地开发如需使用其他已预加载镜像，只能通过显式环境变量覆盖。
...
MVP.1 任务流 app-bundle XCUITest 提供独立显式入口：`MA_NATIVE_APP_TASK_XCUITEST=1 ./platform/native-app/scripts/test-app-bundle.sh` 只选择 `DesignedNativeShellAppBundleTests`；`MA_NATIVE_APP_RUN_TASK_XCUITEST=1 ./platform/native-app/scripts/test.sh` 会先运行默认 fast gate，再追加同一任务 suite。两种入口都依赖 Xcode UI automation 和前台窗口环境，因此都不进入默认快速组件测试；完整 app-bundle suite 仍使用既有 `./platform/native-app/scripts/test-app-bundle.sh` 或 `MA_NATIVE_APP_RUN_XCUITEST=1` 入口。需要先构建当前源码、核对 ad-hoc app/runner identity、但暂不启动 XCTest Automation Mode 时，运行 `MA_NATIVE_APP_TASK_XCUITEST=1 MA_NATIVE_APP_PREPARE_ONLY=1 ./platform/native-app/scripts/test-app-bundle.sh`；该模式只执行 fingerprint-aware `build-for-testing`，要求 `.xctestrun`、目标 `MeetingAssistantNative.app` 和 `MeetingAssistantNativeAppUITests-Runner.app` 三者齐备，分别打印目标 app 与 UI runner 的 path、signature、team、cdhash、designated requirement 和后续运行命令。任一产物缺失时命令非零失败；成功时也不启动 app、testmanagerd 或 test body，不能记作 XCUITest 通过证据。所有共享同一 DerivedData 的 wrapper 运行先取得排他锁；task evidence 模式不接受与当前 app/UI-test 输入 fingerprint 不一致的强制复用，并让 fingerprint 计算、构建、测试与 verifier 使用同一个绝对 `xcodebuild`。每次 attempt 都把 xcresult 写入唯一且拒绝覆盖的 `build/DerivedData/AppBundleUITests/reports/xcresult/<suite>/<run-id>/attempt-<n>.xcresult`，并记录 final attempt。测试前先绑定 `.xctestrun`、target app、runner、test bundle 的严格 codesign/CDHash 和 executable SHA-256；只有 xcodebuild 零退出且离线 verifier 同时证明测试前后 HEAD、fingerprint 与这些工件未变化、精确 17/17 Passed、retained xcresult content manifest 和 8 张可完整解码 PNG 后，wrapper 才成功。上游非零且 final xcresult 存在时，verifier 写出 `passed=false` report，wrapper 始终保留原始退出码；若上游在生成 xcresult 前即失败，则只保留非零退出码和 xcodebuild log，不宣称存在机器报告。导出的私有截图记录 SHA-256，但仍必须逐张人工视觉审查，`screenshot_visual_review=pending` 不能作为视觉验收通过。

`xcresulttool` 在读取或导出时可能物化结果包缓存；因此 verifier 只会在全部读取和导出结束后连续两次采集 retained xcresult content manifest，二者不一致即失败。后续人工证据工具继续精确重算该最终清单；不得忽略缓存文件或放宽变化检测。

MVP.1 截图人工视觉审查在 task suite 成功后，使用 `./scripts/mvp1-screenshot-visual-review.py init <review.json> --task-evidence-report <mvp1-task-xcresult-report.json>` 创建明确未声明的私有模板；真人逐图检查后运行 `validate <review.json> --format json --json-output <validation.json>` 或 `report <review.json> --json-output <report.json> --markdown-output <report.md>`。init 会绑定当前 HEAD、仍为 `screenshot_visual_review=pending` 的 task evidence report bytes、精确 17/17 summary 及八张规定 PNG 的路径与 SHA-256；validate/report 会重新核对这些输入，任何提交、报告、截图或哈希变化均 fail closed。report/PNG 输入以 `O_NOFOLLOW`、`O_NONBLOCK` + `fstat` 在同一 file descriptor 上读取、解析和摘要，strict validator 直接消费该 report bytes，避免检查、解析与 hash 跨不同 bytes；路径保护针对最终文件节点，审查者仍须选择自己控制的父目录。每张截图必须有 pass/fail 和非空 observation；failed screenshot 必须有关联 finding，open P0/P1 阻断，closed P0/P1 必须记录 resolution、通过的真人复测和合成数据/privacy 声明。模板与输出拒绝覆盖、symlink 和特殊文件，并以 `0600` no-clobber 发布。工具会拒绝明确自称 agent、automation、XCUITest 等非真人观察者的角色文本，但仍只能记录人工声明、不能独立证明身份；其 `ready_for_review`、结构有效或 report 都不等于产品验收或 release readiness，agent、自动化、XCUITest 和截图文件本身不能代替真人观察。

真实 task evidence 模式禁止用 `MA_NATIVE_APP_XCODEBUILD_TOOL`、`MA_NATIVE_APP_XCRESULTTOOL`、`MA_NATIVE_APP_CODESIGN_TOOL`、`MA_NATIVE_APP_GIT_TOOL` 或 `MA_NATIVE_APP_IMAGE_DECODE_TOOL` 注入替身。wrapper 必须用固定 `/usr/bin/xcrun` 解析本机生效的 Python runtime、`xcodebuild` / `xcresulttool`，固定 `/usr/bin/python3 -I -S` 执行锁、哈希、冻结和同 bytes loader，忽略 `PYTHONHOME` / `PYTHONPATH` / user-site / `sitecustomize` 注入，并要求这些工具与 `/usr/bin/env`、`xcrun`、`codesign`、`git`、`sips` 全部通过 Apple anchor 校验；capture binding 与最终 report 同时记录调用路径、解析后的真实路径和 executable SHA-256，任一工具变化都阻断。四个 MVP.1 证据脚本的直接 executable 入口也固定以 `/usr/bin/python3 -I -S` 启动。wrapper 不再从 `PATH` 调用 `python3`、`git` 或 `shasum`，Git 调用使用空白 HOME 和最小环境清理 `GIT_DIR` / `GIT_WORK_TREE` / index/object 等重定向；`mvp1-screenshot-visual-review.py`、`mvp1-experience-study.py` 和 `mvp1-accessibility-walkthrough.py` 的 current-HEAD 校验同样固定调用 `/usr/bin/git`，并以最小环境执行，不能被调用者的 `PATH`、Git 配置或 repo/worktree/index/object 重定向影响。verifier 的隐藏 fixture 模式只服务单元测试，即使所有逻辑检查通过也固定写 `passed=false`，不得作为 task evidence。

MVP.1 补充真人体验研究使用 `./scripts/mvp1-experience-study.py init <study.json>` 生成明确未声明的模板，研究人员完成 3–5 名匿名真人的首次、回访和故障恢复任务记录后，再运行 `validate <study.json> --format json` 或 `report <study.json> --json-output <report.json> --markdown-output <report.md>`。validate/report 默认要求 `subject_commit` 匹配当前 HEAD；只有审阅历史研究时才能显式使用 `--allow-historical-subject`，该模式不得关闭当前 `PV-MA-014`。模板不会把空白参与者自动声明为真人，`init` 没有覆盖选项；closed P0/P1 必须记录非空 resolution、人工复测和 passed 结果，open P0/P1 必须阻断。输入不得与输出别名；输出默认拒绝覆盖，已有目录、symlink 或特殊文件不得作为输出，各文件在目标同目录 no-clobber 发布并使用 `0600`；validate/report 显式 `--force` 时先保留已有普通文件，批次任一受控发布失败会回滚，且仍不能覆盖输入或跨输出别名。校验只能确认记录结构、手工声明、人数范围和 finding 状态，不能独立证明真人身份，也不能把 agent、自动化、截图或 XCUITest 转换为真人研究证据。当前没有确认的完成率、时长、求助、误点、犹豫或状态理解通过阈值，工具不得自行添加。

若 force 模式的新输出已经全部发布、但旧备份清理失败，体验研究命令会返回 2，明确说明新输出仍生效并列出保留旧 bytes 的 backup 路径；该状态不声称已经回滚，需人工按报错路径清理。

MVP.1 VoiceOver / 键盘实机走查要在 task suite 成功后，使用 `./scripts/mvp1-accessibility-walkthrough.py init <walkthrough.json> --task-evidence-report <mvp1-task-xcresult-report.json> --tested-app-path <absolute.app> --bundle-id <id> --cdhash <40-hex>` 创建明确未声明的私有模板；工具会在 init、validate 和 report 时要求该 task report 为同一当前 HEAD、稳定 fingerprint、17/17、同一冻结 verifier 与同一真实 app identity，并重新核对严格 codesign、app executable SHA-256、pre-test binding、xctestrun、retained xcresult content manifest 及八张截图 SHA-256，缺失或变化即 fail closed。人工观察者必须在绑定 app 的真实窗口中开启 VoiceOver、仅用键盘完成全部固定检查并逐项写 observation；closed P0/P1 还需 resolution 与通过的直接人工复测。输出拒绝覆盖并使用 `0600`，结构有效或 `ready_for_review` 不等于产品验收或 release readiness。走查必须确认使用合成会议数据；自由文本对邮箱、电话和 raw session id 做保守阻断，但姓名、真实会议内容与完整 transcript 仍依赖人工 privacy 声明，不把自动检查误写成独立隐私证明。

`platform/native-app/scripts/test.sh` 默认运行快速、确定性的 native-app 组件测试入口：component metadata 检查、Swift Testing 和 XCTest-hosted SwiftUI smoke。真实 `.app` 启动的 app-bundle XCUITest 因依赖 Xcode UI automation 和窗口前台状态，默认不进入本地快速 `test` gate；需要完整 native UI 证据时运行 `./platform/native-app/scripts/test-app-bundle.sh`，或设置 `MA_NATIVE_APP_RUN_XCUITEST=1 ./platform/native-app/scripts/test.sh`。无 flag 的完整 suite 也先执行 fingerprint-aware `build-for-testing`，经过与显式 smoke 相同的外部同名实例检查，再用 `test-without-building` 运行全部测试；该入口不新增 Automation Mode 自动重试，仍保留原始测试退出码。`test-app-bundle.sh` 默认使用 `platform/native-app/build/DerivedData/AppBundleUITests` 下的忽略目录复用 Xcode DerivedData；如需一次性隔离构建，可用 `MA_NATIVE_APP_DERIVED_DATA_PATH` 指向临时目录。Debug app bundle 默认 ad-hoc 签名，TCC 授权目标可能随 rebuild 改变；脚本默认 `MA_NATIVE_APP_REUSE_XCTESTRUN=auto`，会对 native app source、UI tests、Xcode project、destination 和 Xcode 版本记录 `.meeting-assistant-xctestrun-inputs.sha256` 指纹，输入未变时自动复用现有 `.xctestrun` 和 app bundle，避免每次 `build-for-testing` 改变签名导致同名 `MeetingAssistantNative` 反复弹 Screen Recording / Screen & System Audio Recording 授权；输入变化或指纹缺失时会 rebuild 一次并刷新指纹，此时可能需要重新授权当前 DerivedData 下的新 app bundle。权限失败 help 会同时打印当前 app bundle 的 bundle id、签名类型、team id、cdhash、designated requirement 和 spctl assessment，避免系统设置中同名 app 条目指向旧 Debug ad-hoc signature 时误判授权已完成。app-bundle XCUITest 启动前还会默认输出 `MA_NATIVE_APP_TCC_IDENTITY_DIAGNOSTICS` 诊断：当当前 `.app` 不是稳定 Apple team identity 签名，或同一 `CFBundleIdentifier` 下存在多个 `MeetingAssistantNative.app` 路径时，脚本会打印当前 path/signature/team/cdhash 和其他路径，说明系统设置中同名条目可能绑定的是旧 worktree 或旧 DerivedData；可用 `MA_NATIVE_APP_TCC_IDENTITY_DIAGNOSTICS=0` 关闭该诊断。可用 `MA_NATIVE_APP_REUSE_XCTESTRUN=0` 强制 rebuild，或用 `MA_NATIVE_APP_REUSE_XCTESTRUN=1` 手动复用现有 `.xctestrun`，但后者在指纹不匹配时只适合复跑已授权的同一 app bundle，不应作为新代码验证证据。显式 app-bundle smoke 对 test body 前的 Automation Mode / LocalAuthentication blocker 默认做 1 次有界重试，可用 `MA_NATIVE_APP_UI_AUTOMATION_RETRY_ATTEMPTS=0` 关闭；真实测试 body 内的 TCC、capture、processing、action 或断言失败不重试，仍保留原退出码。显式设置 `MA_NATIVE_APP_REAL_CAPTURE_SMOKE=1` 时只运行真实 ScreenCaptureKit app-bundle smoke；显式设置 `MA_NATIVE_APP_REAL_PROCESSING_SMOKE=1` 时只运行真实 `processing-cli` app-bundle smoke；显式设置 `MA_NATIVE_APP_REAL_ACTION_SMOKE=1` 时只运行真实 processing artifact -> transcript review -> copy/export/delete action app-bundle smoke；显式设置 `MA_NATIVE_APP_VSMA21_HARDENING_SMOKE=1` 时只运行 VS-MA-21 app-bundle hardening smoke，用于验证 launched `.app` 经 provider-owned fixture 遇到 `path_conflict` 后保留 retry，retry 成功生成派生产物，且原始 `mixed_audio` checksum 未改变；显式设置 `MA_NATIVE_APP_REAL_CAPTURE_SAME_CHAIN_SMOKE=1` 时只运行 VS-MA-21 real capture same-chain app-bundle smoke：launched `.app` 请求真实 ScreenCaptureKit system audio，要求同一 workspace/session 中 `mixed_audio` 为 `available`，再经 provider-owned CLI processing、transcript review、copy/export 和 delete confirmation 串联；若真实 capture 未产出 `mixed_audio`、ffmpeg/媒体标准化不可用、TCC/UI automation 阻断或 processing/action 失败，该命令必须失败并保留原退出码，不能回退到受控音频。显式设置 `MA_NATIVE_APP_MVP_FULL_STACK_SMOKE=1` 时只运行 VS-MA-20 native app-bundle MVP full-stack smoke，证明 designed shell 在同一 workspace/session 中触发受控 native recording、provider-owned CLI processing、transcript review、copy/export 和 delete confirmation。若 macOS 在测试 body 前返回 `LocalAuthentication Code=-4` / `System authentication is running` 或 `Timed out while enabling automation mode`，该运行只能作为 UI automation 环境 blocker，常见原因是 `testmanagerd` 启用 XCTest Automation Mode 时被 `loginwindow` / Touch ID / password authentication 会话抢占，或 Automation Mode 初始化在系统认证未可用时超时；脚本会输出 `loginwindow`、`coreauth*`、`universalAccessAuthWarn`、`online-auth-agent`、`testmanagerd` 和 runner/xcodebuild 的进程诊断以及 `testmanagerd` launchctl 摘要。`MA_NATIVE_APP_REAL_RUNTIME_SMOKE=1` 的 real-runtime 分支还会在任何非零退出时刷新 `reports/ui-automation/real-runtime-ui-automation-report.json`：若已进入 test body 并点击 `ma.processing.startButton` 但 120 秒内未到达真实 runtime 完成状态，report 会记录 `blocker_type=real_runtime_processing_timeout`、`blocked_before_test_body=false`、`processing_start_clicked=true` 和 `processing_completed=false`，该证据仍是 failure/blocker，不得写作真实 runtime app-bundle 通过。需要解除系统认证、Accessibility/Developer Tools 授权提示或真实 runtime 状态/超时问题后重跑。native UI、release-scope 或阶段收口证据不得只引用默认 skip 输出，必须明确记录 app-bundle XCUITest 命令。

上文“test body 前”的判定是 fail-closed 的：只有日志包含 Automation Mode / LocalAuthentication blocker，且尚无任意 `Test Case ... started` 记录时才允许自动重试；一旦 test body 已开始，即使后续日志出现相似文本也保留原失败，不自动重复可能有录制、导出或删除副作用的 smoke。

app-bundle XCUITest 前，`test-app-bundle.sh` 会检查其他路径中是否存在同名 `MeetingAssistantNative.app` 实例。默认 `MA_NATIVE_APP_FOREIGN_INSTANCE_POLICY=fail`：发现其他正在运行的 MeetingAssistantNative 时拒绝启动测试，并要求用户先判断它是否正在录制或保存，不会自动终止或强杀。只有调用方已确认这些实例属于可丢弃的测试进程时，才可显式设置 `MA_NATIVE_APP_FOREIGN_INSTANCE_POLICY=terminate`；诊断场景可显式设为 `ignore`，但可能造成 TCC/Automation 归因干扰。旧 `MA_NATIVE_APP_TERMINATE_STALE_INSTANCES=1/0` 仅保留为显式兼容映射，未设置时不再改变安全默认值。

`./platform/native-app/scripts/run-local-app.sh` 是 VS-MA-23 的本机 local-direct app run 入口，不依赖 XCUITest Automation Mode，也不要求 Developer ID、notarytool、stapler、Sigstore 或 App Store 分发。该脚本默认复用 `.harness/release-build/native-app/DerivedData/Build/Products/Release/MeetingAssistantNative.app`；若 app bundle 不存在，会调用 `./scripts/release-bundle-create.py --distribution-mode local-direct` 构建本机 Release `.app` 和 zip/report。默认启动方式是 fresh LaunchServices `open -n -W -F`，脚本会在 app 运行期间通过 scoped `launchctl setenv` 传入 `MEETING_ASSISTANT_WORKSPACE`、`MEETING_ASSISTANT_TRANSCRIPTION_RUNTIME`、`MEETING_ASSISTANT_TRANSCRIPTION_MODEL`、`MEETING_ASSISTANT_FFMPEG_PATH` 等环境，并绕过本机陈旧 macOS window restoration 状态；退出或中断时恢复原 launchd 环境；`MA_NATIVE_LOCAL_APP_LAUNCH_MODE=direct` 只用于区分本机录制功能与 LaunchServices/TCC attribution 的低层 executable 诊断，默认本机用户路径仍是 LaunchServices `open`。未设置 `MEETING_ASSISTANT_CLI_PATH` 时默认指向 provider-owned `platform/e2e/ma-cli-local.sh`。默认 `MA_NATIVE_LOCAL_APP_REQUIRE_REAL_RUNTIME=1`，缺少 runtime/model 会 fail closed；需要只检查 UI/依赖面时可显式设为 `0`。脚本还会设置普通非 XCTest app 所需的 process/system client：`MA_NATIVE_RECORDING_CLIENT=apple_screencapturekit`、`MA_NATIVE_PROCESSING_CLIENT=process`、`MA_NATIVE_TRANSCRIPT_ACTION_CLIENT=process`、`MA_NATIVE_TRANSCRIPT_ACTION_OS_CLIENT=system`，并默认传入 `MA_NATIVE_PROCESSING_RUNTIME=whisper_cpp` / `MA_NATIVE_PROCESSING_LANGUAGE=zh`。普通非 XCTest app 首次显示 designed shell 时会自动刷新 Preflight，并把真实 `check_dependencies` 结果同步到录制和处理主操作 readiness；XCTest fixture 仍使用静态 Preflight 结果，避免本地依赖状态污染 deterministic UI smoke。可用 `--dry-run` 或 `MA_NATIVE_LOCAL_APP_DRY_RUN=1` 只打印解析后的本机配置。`./platform/native-app/scripts/local-direct-smoke.sh` 会包装该入口，启动 Release app 后用 repo-owned `local-app-ax.swift` 的 CoreGraphics window check 验证 app 拥有可见 layer-0 主窗口，并通过 `MA_NATIVE_LOCAL_APP_SMOKE_STATE_REPORT` 验证 designed shell、Preflight、Recording 和 Processing readiness、主操作控件 identifier/启用状态，以及录制说明文案与 `MA_NATIVE_CAPTURE_SMOKE_SYSTEM_AUDIO` / `MA_NATIVE_CAPTURE_SMOKE_MICROPHONE_AUDIO` 请求一致；该 smoke 不点击录制、不打开 System Settings、不请求或修改权限。`./platform/native-app/scripts/local-direct-recording-smoke.sh` 是更重的 opt-in 本机录制 smoke，会启动同一个 Release app，通过 `AXIdentifier` / `AXPress` 点击 `ma.recording.startButton` 和 `ma.recording.stopButton`，要求 UI 到达 `Recording in progress.`、`Recording saved.`、`screen_video: available` 和 `mixed_audio: available`，并写出 `local-direct-recording-smoke-report.json`；它可能触发 app 的正常 macOS 权限请求路径，但脚本本身不打开 System Settings、不修改 TCC、不要求 Developer ID/公证。`./platform/native-app/scripts/local-direct-processing-smoke.sh` 消费已有 recorded workspace/session，默认读取 `platform/native-app/build/local-direct-recording-smoke-direct/workspace` 和 `session-app-ui-blocked`，启动同一个 Release app 后通过 `AXIdentifier` / `AXPress` 点击 `ma.processing.startButton`，优先要求 UI 到达 `Processing complete.` 或 `Processing completed with transcript-only speaker labels.`，再用结构化 `session.json` / `transcript.json` / `speaker_labels.json` 校验 `normalized_audio`、`transcript_text` 和 `speaker_labels` 产物；如果 macOS AX/window observation 在点击后丢失完成 marker，只有上述结构化产物、expected terms 和 speaker-label fallback 全部通过时才允许 `artifact_completion_fallback=true` 的 report 通过，并保留 `ui_completion_observation_error`。该 smoke 不启动录制、不打开 System Settings、不请求或修改 TCC，也不要求 Developer ID/公证。`./platform/native-app/scripts/local-direct-actions-smoke.sh` 消费已有 processed workspace/session，启动同一个 Release app 后通过 `AXIdentifier` / `AXPress` 点击 copy/export/delete controls，验证 system pasteboard、`NSSavePanel` Markdown export、delete confirmation、外部 export 保留和 delete event 不泄漏 transcript 文本；该 smoke 不启动录制或处理、不打开 System Settings、不请求或修改 TCC，也不要求 Developer ID/公证。`./platform/native-app/scripts/local-direct-same-chain-smoke.sh` 将 recording、processing 和 actions 三段 smoke 绑定到同一个 installed app identity、workspace/session 和默认 `open` launch mode，写出 `local-direct-same-chain-smoke-report.json` 聚合 report；它要求开始前没有既有 `MeetingAssistantNative` 进程，以避免本机 AX/CG app attribution 污染，但不会主动终止用户进程；它可能经 recording stage 触发正常 macOS 权限提示，但 wrapper 本身不打开系统设置、不修改 TCC、不要求 Developer ID/公证。上述 report 会在 LaunchServices 权限失败时给出 direct diagnostic 复跑命令，用于确认是否只是启动归因/TCC 问题；这些入口不证明 XCUITest Automation Mode、跨机器 TCC/display 可重复、release-preflight 或未来 developer-id 商业分发 readiness。

`./platform/e2e/release-local-direct-target-smoke.sh` 是 VS-MA-23 的逐目标机器 local-direct repeatability evidence 入口。它调用 `./platform/native-app/scripts/local-direct-same-chain-smoke.sh`，保留 same-chain log，并用 `release_local_direct_target_smoke_report.py` 生成 `release_gate=release-local-direct-target-smoke` report；report 绑定当前 commit、target id/os/architecture、installed app identity、same-chain report digest、三段 stage 结果、默认 `open` launch mode、expected transcript terms、system pasteboard、`NSSavePanel` export 和 delete markers。该入口只证明当前 target machine，不声明 `target_scope=all-target-machines`，不打开 System Settings、不修改 TCC、不要求 Developer ID/公证，也不证明 `release-preflight` 或商业分发 readiness。

local-direct same-chain 使用音频 fixture 时，`local-direct-recording-smoke.sh` 会先满足 `MA_NATIVE_LOCAL_APP_RECORDING_SMOKE_DURATION_SECONDS`，再等待 fixture 在 `MA_NATIVE_LOCAL_APP_RECORDING_SMOKE_AUDIO_GRACE_SECONDS` 内播放完成，避免 transcript expected terms 被提前 stop 截断。`local-direct-processing-smoke.sh` 优先使用 UI completion marker；若点击 Start Processing 后 macOS AX/window observation 丢失 completion marker，只有结构化 `normalized_audio`、`transcript_text`、`speaker_labels`、expected terms 和 speaker-label fallback 全部通过时，才允许 `artifact_completion_fallback=true` 的 report 通过，并保留 `ui_completion_observation_error`。
...
`./platform/e2e/local-direct-functional-preflight.sh` 是 VS-MA-23 的本机功能 preflight 入口，不属于 release-preflight。它默认先运行 `release-bundle-create.py --distribution-mode local-direct` 刷新当前 source Release app 和 bundle report，但不安装或重签本机 app；随后顺序运行 `release-local-direct-target-smoke.sh` 和单台 expected target 的 `release-local-direct-repeatability-report.sh`，再调用 `local_direct_functional_preflight_report.py` 生成 `release_gate=local-direct-functional-preflight`、`target_scope=local-machine-only` report。即使 target smoke 因 TCC、LaunchServices attribution、recording、processing 或 action blocker 失败，wrapper 也会继续生成当前 HEAD 的 failure report，再保留原失败退出码；target/preflight report 会把失败 stage 的短 `failure_context` 上冒到 `findings`，例如 `permission_denied`、exact app path 和 stable codesign designated requirement，避免 `product-validation-check.py local-functional` 回退到旧 report 或输出无关字段缺失。该 report 会校验 bundle report 绑定当前 HEAD，且已安装 local-direct app 的 unsigned executable SHA256 与当前 source Release app unsigned executable SHA256 一致；不一致时 fail closed 并要求重新运行 `install-local-app.sh --rebuild`。该入口用于确认当前已授权本机 installed local-direct app 的 recording -> processing -> transcript actions 链路可复跑，并且显式保留 `not_release_readiness=true`；它不打开 System Settings、不修改 TCC、不要求 Developer ID/公证。单独的 local-functional report 不能替代 `product-validation-check.py release` 或最终 `release-preflight.sh`。

`./scripts/product-validation-check.py local-functional` 是本机功能验收视图。它先执行 `current-phase` 的矩阵文档完整性检查，再读取 `MA_LOCAL_DIRECT_FUNCTIONAL_PREFLIGHT_REPORT` 指定的 report，或默认读取最新 `platform/e2e/build/local-direct-functional-preflight/reports/local-direct-functional-preflight-*.json`。该命令要求 report 绑定当前 HEAD、`release_gate=local-direct-functional-preflight`、`target_scope=local-machine-only`、`passed=true`、`not_release_readiness=true`、LaunchServices `open`、同一 installed app identity、当前 local-direct bundle report、source/installed unsigned executable SHA256 match、recording/processing/actions 三段通过、expected transcript terms 命中以及 copy/export/delete marker 存在；当当前 report `passed=false` 且包含 `findings` 时，命令优先输出这些 current preflight findings，而不是继续列出后续字段缺失噪声。它用于回答“当前本机 installed app 功能是否跑通”；它不能替代 `product-validation-check.py release` 或 `release-preflight.sh`。

`./platform/native-app/scripts/install-local-app.sh` 是 VS-MA-23 的 local-direct app install 入口，默认把本机 Release app 安装到稳定用户路径 `~/Applications/MeetingAssistantNativeLocal.app`。该脚本可用 `--rebuild` 先刷新 `.harness/release-build/.../MeetingAssistantNative.app`，复制前校验 source app 仍为 `CFBundleIdentifier=local.meeting-assistant.native`、`CFBundleName=MeetingAssistantNative`、app executable 和本地 `codesign --verify --deep --strict`；复制后只改安装副本的本机身份，默认写入 `CFBundleIdentifier=local.meeting-assistant.native.localdirect`、`CFBundleName=MeetingAssistantNativeLocal`、`CFBundleDisplayName=Meeting Assistant Native Local`。默认 `MA_NATIVE_LOCAL_APP_SIGNING_MODE=stable-local` 会创建或复用项目专用本机自签名 Code Signing identity（默认 keychain 为 `~/Library/Keychains/meeting-assistant-local-signing.keychain-db`，密码文件位于 `~/Library/Application Support/MeetingAssistant/local-signing/`），再用该 identity 签名安装副本，使 designated requirement 绑定 `CFBundleIdentifier` 和稳定 certificate root，而不是 rebuild-specific ad-hoc `CDHash`；这解决“系统设置里已授权但 rebuild 后又被当作新 app”的本机 TCC 身份问题。`MA_NATIVE_LOCAL_APP_SIGNING_MODE=ad-hoc` 只作为显式低层诊断 fallback，不应作为需要持久录屏授权的本机安装方式。已存在安装目标必须是旧 source identity 或新的 local-direct identity 才会替换，避免覆盖无关应用。安装成功后写出 `platform/native-app/build/local-app-install/local-app-install-report.json`，记录 `release_gate=local-direct-app-install`、source app identity、installed app identity、codesign `CDHash`、designated requirement、`local_signing`、`local_tcc_identity_strategy`、`opens_system_settings=false`、`modifies_tcc_or_system_settings=false`、`requires_developer_id_or_notarization=false`、`not_release_readiness=true` 和 direct recording diagnostic 复跑命令。该脚本不打开 System Settings、不修改 TCC、不调用 `tccutil`、不要求 Developer ID/公证/Stapler/Sigstore/App Store 分发；它只把本机安装授权对象从发布签名问题里隔离出来。安装后可用 `MA_NATIVE_LOCAL_APP_PATH="$HOME/Applications/MeetingAssistantNativeLocal.app" ./platform/native-app/scripts/run-local-app.sh --no-build` 或 `./platform/native-app/scripts/local-direct-smoke.sh --no-build --app "$HOME/Applications/MeetingAssistantNativeLocal.app"` 针对稳定安装路径复跑。

`./platform/native-app/scripts/local-app-permission-diagnostics.sh` 是 VS-MA-23 的只读权限诊断入口，读取 `local-app-install-report.json`，并在未显式传入 `--recording-report` 时从 `platform/native-app/build/` 和 `platform/e2e/build/` 下选择最新 `local-direct-recording-smoke-report.json`，写出 `platform/native-app/build/local-app-permission-diagnostics/local-app-permission-diagnostics-report.json`。该 report 记录 recommended TCC target path/`CFBundleDisplayName`/CDHash、install report 的 `local_tcc_identity_strategy`、recording smoke 是否使用同一 app identity、是否命中 Screen Recording denied、是否需要用户动作、授权后应复跑的 local-direct recording smoke 命令，以及 direct launch diagnostic 命令；若同一 app path/CDHash 的 direct launch recording smoke 通过而 LaunchServices `open` recording smoke 被 `permission_denied`，report 会设置 `launchservices_tcc_attribution_suspected=true` 并记录对应 direct/open report 路径。脚本不打开 System Settings、不修改 TCC、不调用 `tccutil`、不要求 Developer ID/公证/Stapler/Sigstore/App Store 分发；它只能把本机授权目标、LaunchServices/TCC attribution 诊断和下一步复跑命令机器可读化，不证明授权已完成或 release readiness。
