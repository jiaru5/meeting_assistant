...
MVP.1 截图人工视觉审查在 task suite 成功后，使用 `./scripts/mvp1-screenshot-visual-review.py init <review.json> --task-evidence-report <mvp1-task-xcresult-report.json>` 创建明确未声明的私有模板；真人逐图检查后运行 `validate <review.json> --format json --json-output <validation.json>` 或 `report <review.json> --json-output <report.json> --markdown-output <report.md>`。init 会绑定当前 HEAD、仍为 `screenshot_visual_review=pending` 的 task evidence report bytes、精确 17/17 summary 及八张规定 PNG 的路径与 SHA-256；validate/report 会重新核对这些输入，任何提交、报告、截图或哈希变化均 fail closed。每张截图必须有 pass/fail 和非空 observation；failed screenshot 必须有关联 finding，open P0/P1 阻断，closed P0/P1 必须记录 resolution、通过的真人复测和合成数据/privacy 声明。模板与输出拒绝覆盖、symlink 和特殊文件，并以 `0600` no-clobber 发布。工具只使人工记录可审计：其 `ready_for_review`、结构有效或 report 都不等于产品验收或 release readiness，agent、自动化、XCUITest 和截图文件本身不能代替真人观察。
