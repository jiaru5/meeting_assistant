...
| `VS-MA-23` | partial evidence | `release-bundle-create.py` / `release-candidate-inputs.py` 默认 local-direct 已能构建并校验本机 Release app bundle；`product-validation-check.py release` 仍有发布范围 `partial` 行 | 继续关闭剩余 `PV-MA-*` partial 行，并让 local-direct `release-preflight.sh` 最终通过 |
