...
| `VS-MA-21` | partial evidence | processing failure retry、native safe display、stop retry、restart recovery、路径/日志脱敏，以及 capture-style lock conflict -> retry success/checksum preserved E2E 证据已有；`platform/e2e/release-capture-processing-hardening-smoke.sh` 和结构化 report 声明 `release_gate=release-scope-provider-hardening`，覆盖 provider-side no-auto-download、path/lock/temp/checksum rollback、failure redaction、retry success 和 checksum preservation 标准门禁；Debug/XCTest-only `MA_NATIVE_APP_VSMA21_HARDENING_SMOKE=1 ./platform/native-app/scripts/test-app-bundle.sh` 入口和 fixture 已存在；2026-07-06 07:48 CST 本机复跑通过，单个 app-bundle XCUITest 从 launched `.app` 点击 `ma.processing.startButton`，观察 `path_conflict` / `Processing failed.` / retry button，点击 retry 后进入 `Processing complete.` 并验证原始 `mixed_audio` checksum preservation；`MA_NATIVE_VSMA21_HARDENING_BRIDGE_SMOKE=1 ./platform/native-app/scripts/test.sh` 也已在 06:59 CST 通过；新增 `platform/e2e/release-native-ui-hardening-smoke.sh` 和 report 生成器，把 real capture app-bundle 与 VS-MA-21 app-bundle hardening 聚合为 release-scope native UI hardening gate；2026-07-06 07:58 CST 本机运行 fail closed 在 real capture `permission_denied`，hardening stage 通过 | 恢复/授予本次 DerivedData app bundle 的 Screen Recording / Screen & System Audio Recording 权限后重跑 `release-native-ui-hardening-smoke.sh`；补真实 capture -> transcript/export/delete 同链路和 release bundle 证据；当前不得外推为 `PV-MA-009` covered 或 `VS-MA-23` readiness |
| `VS-MA-22` | partial evidence | 组件 security/SBOM/build/architecture、provider redaction、SBOM Apache-2.0 license/no packaged third-party runtime component/validation-only scope，以及 model sidecar hash mismatch / invalid provenance / sidecar root fail-closed 证据已有；本轮新增仓库级 `security/security-report.json` 聚合校验，`security-check.sh` 会删除旧报告、运行每个 production component security gate，并要求报告声明 SAST/SCA/secret/no-auto-download/无外部网络/未打包 runtime/model/零 findings；`supply-chain-check.sh current` 现在同样删除旧 `supply-chain/supply-chain-report.json`、运行每个 production component SBOM gate，并要求报告声明 CycloneDX、Apache-2.0、SCA/license review、validation-only provenance scope、无 packaged runtime、无自动下载和零 findings；release-provider smoke 的非契约 marker 已调整为 `VS-MA-22`，组件测试新增 runtime/model/audio 不在 `.local` 允许根或 English-only `.en` 模型时 fail-closed 证据；本轮新增 `platform/e2e/release-security-supply-chain-smoke.sh`，聚合 `security-check`、`supply-chain-check current` 和 `release-provider-smoke`，report 声明 `release_gate=release-scope-security-supply-chain` 且 `not_release_readiness=true`；`supply-chain-check.sh release` 现在要求 release bundle/provenance/signing/sidecar evidence report，且 provenance/signature artifact digest 必须匹配 release bundle report 的同一 bundle digest；同时 provenance report 必须指向 digest 匹配、可解析的 DSSE in-toto SLSA provenance v1 attestation，signature report 必须为同一 artifact 提供 keyless OIDC certificate identity/issuer、transparency log 和 digest 匹配的 Sigstore bundle；缺少 CI/人工 release rehearsal 产出的 bundle/SLSA/signing/target-machine sidecar repeatability report，SLSA/signing report 指向另一份 release artifact，或只提供不可审计 JSON 声明时 fail closed，避免把 validation-only SBOM 或单机 provider sidecar smoke 误当 release 放行；`release-bundle-check.sh` 现在要求 release bundle report 绑定当前 commit、digest、Release app zip archive 和 no-runtime/model/no-meeting-data 声明，并会解压唯一 `MeetingAssistantNative.app` 后运行 `codesign`、`stapler` 和 `spctl`，缺少真实签名/公证/staple 的 release bundle evidence 时 fail closed；Debug/XCTest-only real runtime app-bundle smoke 入口已存在，`.app` 可在显式 `MA_NATIVE_APP_REAL_RUNTIME_SMOKE=1` 下把 Start Processing 默认参数切到 `runtime=whisper_cpp` / `language=zh` 并继续经 provider-owned `platform/e2e/ma-cli-local.sh` 执行；`platform/e2e/native_app_bundle_ui_automation_report.py` 会把 real-runtime 非零退出分为 `local_authentication_in_progress`、`automation_mode_unavailable` 或 `real_runtime_processing_timeout`，并记录 `blocked_before_test_body`、`processing_start_clicked`、`processing_completed` 和安全诊断路径；`MA_NATIVE_REAL_RUNTIME_BRIDGE_SMOKE=1 ./platform/native-app/scripts/test.sh` fast-gate 已证明 `ProcessingStateViewModel -> ProcessingCommandProcessRunner -> provider-owned CLI -> whisper.cpp -> TranscriptReviewWorkspaceLoader` 可用，真实 transcript 含中文和英文技术词，且 runtime/model/audio 只由显式 env 注入；2026-07-05 23:56 复跑 `MA_NATIVE_APP_XCODE_DESTINATION='platform=macOS,arch=arm64' MA_NATIVE_APP_REAL_RUNTIME_SMOKE=1 ./platform/native-app/scripts/test-app-bundle.sh` 已通过，单个 XCUITest 证明 `.app` UI 点击同一 workspace/session 的 start processing 后可经真实 `whisper.cpp` runtime 生成 transcript，并由 transcript review 读回 `transcript-only fallback` | 产出真实签名/公证 release bundle、SLSA/release provenance attestation、release signature report 和 release sidecar repeatability report，并确保三者绑定同一 release bundle digest；该 security/supply-chain report、release bundle guard、release bundle-bound provenance/signing/sidecar guard、native real-runtime bridge 和 Debug/XCTest opt-in app-bundle real-runtime smoke 仍不得外推为真实 ScreenCaptureKit、真实 Save Panel、PV 全 covered 或 release readiness |
...
2026-07-06 VS-MA-22 sidecar portability 补充：新增 `platform/e2e/release-sidecar-portability-report.sh` 和 `platform/e2e/release_sidecar_portability_report.py`，用于把发布 rehearsal/CI 收集到的逐目标机器 `release-sidecar-target-smoke` report 聚合为 `.harness/release-inputs/supply-chain/release-sidecar-report.json`。该入口强制要求显式 `MA_RELEASE_SIDECAR_EXPECTED_TARGETS`，并在缺 target、额外 target、target report 未通过或 report commit 不匹配时写出 `target_scope=incomplete-target-set` 并失败。该补充关闭“all-target sidecar report 生成器和 fail-closed 规则”这一工程缺口，但仍需要真实 release rehearsal 产出完整 target report 集合、签名/公证 release bundle、SLSA provenance 和 Sigstore signing 后，`VS-MA-22` 才能脱离 `partial evidence`。
