...
| `VS-MA-21` | partial evidence | processing failure retry、native safe display、stop retry、restart recovery、路径/日志脱敏，以及 capture-style lock conflict -> retry success/checksum preserved E2E 证据已有；本轮新增 `platform/e2e/release-capture-processing-hardening-smoke.sh` 和结构化 report，声明 `release_gate=release-scope-provider-hardening`，覆盖 provider-side no-auto-download、path/lock/temp/checksum rollback、failure redaction、retry success 和 checksum preservation 标准门禁 | 补真实 ScreenCaptureKit/native UI release-scope 并发、真实 capture -> transcript/export/delete 同链路和 release bundle 证据；当前不得外推为 `PV-MA-009` covered 或 `VS-MA-23` readiness |
