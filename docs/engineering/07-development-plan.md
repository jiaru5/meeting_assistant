...
| `VS-MA-22` | partial evidence | 组件 security/SBOM/build/architecture、provider redaction、SBOM Apache-2.0 license/no packaged third-party runtime component/validation-only scope，以及 model sidecar hash mismatch / invalid provenance / sidecar root fail-closed 证据已有；本轮新增仓库级 `security/security-report.json` 聚合校验，`security-check.sh` 会删除旧报告、运行每个 production component security gate，并要求报告声明 SAST/SCA/secret/no-auto-download/无外部网络/未打包 runtime/model/零 findings；Debug/XCTest-only real runtime app-bundle smoke 入口已存在，`.app` 可在显式 `MA_NATIVE_APP_REAL_RUNTIME_SMOKE=1` 下把 Start Processing 默认参数切到 `runtime=whisper_cpp` / `language=zh` 并继续经 provider-owned `platform/e2e/ma-cli-local.sh` 执行，`build-for-testing` 已通过，但当前本机 `testmanagerd` / Automation Mode 在 test body 前超时，不能记为通过证据；`MA_NATIVE_REAL_RUNTIME_BRIDGE_SMOKE=1 ./platform/native-app/scripts/test.sh` fast-gate 已证明 `ProcessingStateViewModel -> ProcessingCommandProcessRunner -> provider-owned CLI -> whisper.cpp -> TranscriptReviewWorkspaceLoader` 可用，真实 transcript 含中文和英文技术词，且 runtime/model/audio 只由显式 env 注入 | 补完整 release-scope SAST/SCA/license/provenance 和 model sidecar 证据；解除本机 macOS UI automation blocker 后重跑 real runtime app-bundle smoke；security report 和 native bridge smoke 仍是 current/check scope 或非 `.app` 证据，不得外推为真实 ScreenCaptureKit、真实 Save Panel、release bundle 或 release readiness |
