...
| `VS-MA-17` | partial evidence | transcript review UI 和 workspace artifact loader 已能消费 fixture/受控 artifact；新增 real action app-bundle smoke 路由会先经 `.app` UI 触发真实 `processing-cli` 生成 transcript/speaker artifacts，再重启 `.app` 从同一 workspace/session 回查 transcript，但本机 `MA_NATIVE_APP_REAL_ACTION_SMOKE=1 ./platform/native-app/scripts/test-app-bundle.sh` 当前在测试 body 前被 XCTest Automation Mode / LocalAuthentication 阻断；清理 `universalAccessAuthWarn`、`coreautha` 和 `testmanagerd` 后仍复现 `LocalAuthentication Code=-4 "System authentication is running"`，脚本已输出进程和 `testmanagerd` launchctl 诊断 | 解除本机系统认证 / Automation Mode 阻塞后重跑 real action app-bundle smoke，取得 transcript review 执行通过证据 |
| `VS-MA-18` | partial evidence | copy/export UI、受控 process-runner workspace、production OS pasteboard/save-panel boundary 已有证据；新增 real action app-bundle smoke 已把 copy/export 接到 provider-owned `platform/e2e/ma-cli-local.sh` action route，并断言导出文件保留，但当前执行同样被 Automation Mode / LocalAuthentication 阻断在测试 body 前 | 补 real action app-bundle smoke 执行通过证据；仍不把 deterministic XCTest save destination 等同于真实用户 Save Panel |
| `VS-MA-19` | partial evidence | delete confirmation UI、受控 process-runner 删除、安全错误展示和 production command-client default 已有证据；新增 real action app-bundle smoke 已把 delete 接到 provider-owned `platform/e2e/ma-cli-local.sh` action route，并断言 session 删除、workspace 外 export 保留和 delete event 不泄漏 transcript，但当前执行同样被 Automation Mode / LocalAuthentication 阻断在测试 body 前 | 补 real action app-bundle smoke 执行通过证据；随后再由 VS-MA-20 串同一条 release-scope full-stack 链路 |
