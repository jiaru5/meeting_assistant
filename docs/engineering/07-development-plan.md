...
| `VS-MA-23` | partial evidence | `release-bundle-create.py` / `release-candidate-inputs.py` 默认 local-direct 已能构建并校验本机 Release app bundle；`install-local-app.sh` 已把本机安装目标收敛到 `~/Applications/MeetingAssistantNativeLocal.app`、`CFBundleIdentifier=local.meeting-assistant.native.localdirect` 和本地 ad-hoc signing；`local-direct-smoke.sh` 已证明 installed app 可通过 LaunchServices 普通路径启动、显示可见主窗口并同步 Preflight/Recording/Processing ready 状态；新增本机功能优先的 `local-direct-recording-smoke.sh` / `local-direct-processing-smoke.sh` 入口用于普通 app 路径下点击录制和处理；`product-validation-check.py release` 仍有发布范围 `partial` 行 | 签名、公证、App Store/商业分发继续放到 MVP 外；下一步先在 `MeetingAssistantNativeLocal.app` 上跑通本机录制 Start/Stop -> artifacts -> real runtime processing -> transcript review/copy/export/delete，再继续关闭跨机器重复性、PV covered 和最终 release gate |
