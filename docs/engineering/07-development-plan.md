...
| `VS-MA-22` | partial evidence | 组件 security/SBOM/build/architecture、provider redaction、SBOM Apache-2.0 license/no packaged third-party runtime component/validation-only scope，以及 model sidecar hash mismatch / invalid provenance / sidecar root fail-closed 证据已有；本轮新增 Debug/XCTest-only real runtime app-bundle smoke 入口，`.app` 可在显式 `MA_NATIVE_APP_REAL_RUNTIME_SMOKE=1` 下把 Start Processing 默认参数切到 `runtime=whisper_cpp` / `language=zh` 并继续经 provider-owned `platform/e2e/ma-cli-local.sh` 执行，`build-for-testing` 已通过，但当前本机 `testmanagerd` / Automation Mode 在 test body 前超时，不能记为通过证据 | 补完整 release-scope SAST/SCA/license/provenance 和 model sidecar 证据；解除本机 macOS UI automation blocker 后重跑 real runtime app-bundle smoke，仍不得把 opt-in app-bundle 证据外推为 release readiness |
