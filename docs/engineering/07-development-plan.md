...
4. `local-direct-actions-smoke.sh` 已补齐 transcript review、copy/export/delete 的普通用户链路证据；`local-direct-same-chain-smoke.sh` 已把上述三段收敛为单命令可复跑证据。下一步回到跨机器重复性、PV covered 和 release-preflight。
