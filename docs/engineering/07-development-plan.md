...
| `VS-MA-21` | partial evidence | processing failure retry、native safe display、stop retry、restart recovery、路径/日志脱敏，以及 capture-style lock conflict -> retry success/checksum preserved E2E 证据已有 | 在真实 capture/native-processing 链路闭合后补 release-scope 并发、重试和 checksum 标准门禁证据 |
