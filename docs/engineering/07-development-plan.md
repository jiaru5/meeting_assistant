...
| `VS-MA-21` | partial evidence | processing failure retry、native safe display、stop retry、restart recovery、路径/日志脱敏，以及 capture-style lock conflict -> retry success/checksum preserved E2E 证据已有；`platform/e2e/release-capture-processing-hardening-smoke.sh` 和结构化 report 声明 `release_gate=release-scope-provider-hardening`，覆盖 provider-side no-auto-download、path/lock/temp/checksum rollback、failure redaction、retry success 和 checksum preservation 标准门禁；本轮新增 Debug/XCTest-only `MA_NATIVE_APP_VSMA21_HARDENING_SMOKE=1 ./platform/native-app/scripts/test-app-bundle.sh` 入口和 fixture，用于 app-bundle UI path_conflict -> retry success + 原始 `mixed_audio` checksum preservation 验证，并在非零退出时写出 `vs-ma-21-hardening-ui-automation-report.json`；2026-07-06 本机复跑在 test body 前被 XCTest Automation Mode timeout 阻断，report 声明 `blocked=true`、`blocker_type=automation_mode_unavailable`、`proves_ui_or_provider_behavior=false` | 解除本机 UI automation blocker 后重跑 VS-MA-21 app-bundle hardening smoke；补真实 ScreenCaptureKit/native UI release-scope 并发、真实 capture -> transcript/export/delete 同链路和 release bundle 证据；当前不得外推为 `PV-MA-009` covered 或 `VS-MA-23` readiness |
...
2026-07-06 VS-MA-21 native hardening bridge 补充：本机复跑 `MA_NATIVE_APP_XCODE_DESTINATION='platform=macOS,arch=arm64' MA_NATIVE_APP_VSMA21_HARDENING_SMOKE=1 ./platform/native-app/scripts/test-app-bundle.sh` 仍在 XCUITest test body 前被 Automation Mode timeout 阻断。为补强非 XCUITest 的 native bridge 证据，新增 `MA_NATIVE_VSMA21_HARDENING_BRIDGE_SMOKE=1 ./platform/native-app/scripts/test.sh` fast-gate，使用 native-owned `path-conflict-then-success` fixture 证明 `ProcessingStateViewModel -> ProcessingCommandProcessRunner` 在 `path_conflict` 后保留 retry、retry 成功生成 transcript/speaker labels，且原始 `mixed_audio` checksum 不变。2026-07-06 06:59 CST 本机执行 `MA_NATIVE_VSMA21_HARDENING_BRIDGE_SMOKE=1 ./platform/native-app/scripts/test.sh` 通过，新增用例输出 `VS-MA-21 native-app hardening bridge marker [non-contract]`。该补充不替代 launched `.app` UI hardening、真实 ScreenCaptureKit/native UI release-scope 并发、真实 capture -> transcript/export/delete 同链路、release bundle 或 `PV-MA-009` covered；`VS-MA-21` 继续保持 `partial evidence`。
