...
2026-07-06 VS-MA-23 local sidecar input 补充：14:58 CST 本机运行 `./platform/e2e/release-sidecar-target-smoke.sh` 通过，target report 绑定 `syn-172-254-243-004-macos-arm64`、当前 commit、`.local` runtime/model/audio digest、model license/provenance 和三项 smoke；随后用该 report 和 `MA_RELEASE_SIDECAR_EXPECTED_TARGETS=syn-172-254-243-004-macos-arm64` 运行 `./platform/e2e/release-sidecar-portability-report.sh` 通过，写出默认 `.harness/release-inputs/supply-chain/release-sidecar-report.json`。同轮 `./scripts/supply-chain-check.sh release` 不再报告 sidecar 缺失，只剩 release bundle、provenance 和 signature report 缺失。该补充只证明当前单目标 release rehearsal 的 sidecar portability 输入已具备；签名/公证 release bundle、bundle-bound DSSE/SLSA provenance、Sigstore signing、目标集合扩展后的机器证据和最终 `release-preflight` 仍属于 VS-MA-23 release candidate 输入。
