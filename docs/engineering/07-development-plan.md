...
本快照以 `develop@c8fd019` 为起点复核。`./scripts/vs-stage-check.py current-phase` 和 `./scripts/vs-stage-check.py release` 均通过，输出 `已达退出口径=22`、`partial evidence=3`、`MVP 外=2`；`./scripts/product-validation-check.py release` 仍 fail closed，输出 `covered=14, partial=9`，说明 VS 阶段状态已允许进入 PV 收口分析，但产品 release 仍不能放行。
...
| partial evidence | `VS-MA-06`, `VS-MA-08`, `VS-MA-23` | 状态表和 `product-validation-check.py release` | `VS-MA-06/08` 的剩余风险不阻塞本机 MVP 主链路；`VS-MA-23` 仍受 `PV-MA-*` partial 行阻断 |
...
| `VS-MA-23` | partial evidence | `release-bundle-create.py` / `release-candidate-inputs.py` 默认 local-direct 已能构建并校验本机 Release app bundle；新增本机功能优先的 `MA_NATIVE_APP_REAL_CAPTURE_REAL_RUNTIME_SAME_CHAIN_SMOKE=1 ./platform/native-app/scripts/test-app-bundle.sh`，用于把 launched `.app` 真实 ScreenCaptureKit `mixed_audio` 接到真实 `whisper_cpp` runtime、transcript review 和 transcript actions；`product-validation-check.py release` 仍有发布范围 `partial` 行 | 签名、公证、App Store/商业分发继续放到 MVP 外；下一步先解除本机 XCUITest Automation Mode blocker，复跑 real capture + real runtime same-chain，再继续关闭剩余 `PV-MA-*` partial 行 |
