...
2026-07-06 VS-MA-21 audio-chain guard 补充：`release_native_ui_hardening_report.py` 现在会解析 `AppleScreenCaptureKitNativeCaptureAdapter.swift` 的 capability summary，结构化记录当前真实 capture adapter 是 combined recording file 支持、`producesSeparateAudioArtifacts=false`，并显式输出 `real_capture_independent_audio_artifacts_proven=false`、`real_capture_mixed_audio_artifact_proven=false` 和 `real_capture_to_processing_same_chain_proven=false`。如果该 source contract 变化而 report 仍未更新，会以 `adapter_capability_contract_changed` fail closed。该补充不关闭 TCC blocker，也不把 `VS-MA-21` 提升到退出口径；它只防止 real capture pass 被误读为独立音频产物或真实 capture -> transcript/export/delete 同链路 release 证据。
