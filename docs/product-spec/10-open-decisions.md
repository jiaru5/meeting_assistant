...
|---|---|---|---|---|---|---|
| OD-MA-001 | closed | 原生 macOS 录制的技术边界是什么？ | `04-user-journeys-and-ui.md`, `08-implementation-guidance.md`, manifest | 决策：设计化 Swift/SwiftUI native app shell + local helper / processing CLI；实现早期的最小/调试 UI 只作为中间形态；具体 capture API 以 adapter 方式落地，不改变 artifact contract | JeRRy | 已写入旅程、implementation guidance 和 ADR |
...
| OD-MA-005 | closed | MVP control surface 是完整桌面 UI、菜单栏工具、最小窗口、CLI/helper，还是组合？ | `04-user-journeys-and-ui.md`, `12-ui-ux-design.md`, manifest | 决策：设计化 Swift/SwiftUI native app shell + local helper / processing CLI；primitive/debug native UI 不是 MVP UI 完成口径 | JeRRy | 已写入 UI/UX 和旅程分卷 |
