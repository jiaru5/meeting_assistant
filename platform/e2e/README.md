...
`full-stack-smoke.sh` 是 manifest 注册入口，顺序运行 import processing local smoke、native_recording-style provider smoke 和 native bridge smoke，并在最终 pass marker 前输出 VS-MA-20 provider/e2e attribution summary。`platform/processing-cli/scripts/test.sh` 仍直接调用 processing-owned smoke，避免 processing 组件测试反向依赖 native/Xcode 工具链。
