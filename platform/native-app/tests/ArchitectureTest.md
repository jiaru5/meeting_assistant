...
10. `scripts/local-app-permission-diagnostics.sh` may read the local app install report and latest local-direct recording smoke report, then write a read-only diagnostic report identifying the recommended TCC target path/CDHash, whether the recording smoke used the same app identity, whether Screen Recording was denied, and the recommended smoke command to rerun. It must not open System Settings, modify TCC, require Developer ID/notarization, or claim release readiness.
