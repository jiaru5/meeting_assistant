...
6. The app root must use `App.init` to install a launch coordinator that opens a retained AppKit window controller after `NSApplication.didFinishLaunchingNotification`, not an AppDelegate callback or SwiftUI `WindowGroup` auto-open. The main window must host `NativeControlPlaneRootView` through `NSHostingController(rootView:)`, use content size 1180x760, set frame autosave name `meeting-assistant-main`, and call `window.makeKeyAndOrderFront(nil)` on launch/reuse; the coordinator must also reopen the retained window when the app becomes active and no primary window is visible. This avoids a live process with no primary window when a Settings-only SwiftUI app does not trigger AppDelegate or WindowGroup content creation.
...
14. `scripts/local-direct-recording-smoke.sh` may wrap `run-local-app.sh`, press `ma.recording.startButton` and `ma.recording.stopButton` through macOS Accessibility `AXIdentifier` / `AXPress`, and verify `Recording in progress.`, `Recording saved.`, `screen_video: available` and `mixed_audio: available` markers. It may trigger the app's normal permission request path through user-action recording, but the script itself must not open System Settings, modify TCC, require distribution gates, or claim release readiness. Its report must include the resolved recording request flags, app identity, permission failure details, stale TCC identity hint, direct launch diagnostic hint, and workspace precondition; reused smoke workspaces that hit the fixed local app smoke session id must be classified as `path_conflict`, not a generic UI timeout.
15. `scripts/local-direct-processing-smoke.sh` may wrap `run-local-app.sh`, pass an existing recorded workspace/session into the launched Release app, press `ma.processing.startButton` through macOS Accessibility `AXIdentifier` / `AXPress`, and verify `Processing complete.` or `Processing completed with transcript-only speaker labels.` plus `normalized_audio`, `transcript_text` and `speaker_labels` artifacts. It consumes existing recording artifacts and must not start recording, open System Settings, request or modify macOS permissions, require Developer ID/notarization, or claim release readiness. Its report must include the workspace precondition, recording input boundary, app identity, launch attribution boundary and processed artifact summary.
16. `scripts/local-direct-actions-smoke.sh` may wrap `run-local-app.sh`, pass an existing processed workspace/session into the launched Release app, press `ma.transcriptAction.copyButton`, `ma.transcriptAction.exportButton`, `ma.transcriptAction.deleteButton` and `ma.transcriptAction.deleteConfirmButton` through macOS Accessibility `AXIdentifier` / `AXPress`, and verify `Transcript actions are ready.`, `Copy complete.`, `Export complete.` and `Delete complete.`. It must validate system `NSPasteboard` content through `pbpaste`, confirm the production `NSSavePanel` export UI, verify the external Markdown export remains after delete, and verify the delete event does not leak transcript text. It consumes existing processing artifacts and must not start recording, start processing, open System Settings, request or modify macOS permissions, require Developer ID/notarization, or claim release readiness. Its report must include the workspace precondition, action boundary, app identity, launch attribution boundary, export path, retained export status and delete event path.
