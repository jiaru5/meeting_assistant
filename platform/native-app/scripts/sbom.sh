...
if component.get("name") != "meeting-assistant-native-app":
...
if "Apache-2.0" not in component_license_ids:
    raise SystemExit("native-app sbom failed: component license evidence must include Apache-2.0")
...
for name, expected in required_properties.items():
    if properties.get(name) != expected:
        raise SystemExit(f"native-app sbom failed: metadata property {name} must be {expected}")
