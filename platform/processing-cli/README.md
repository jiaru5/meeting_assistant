...
当前目录没有公开 `normalize_audio` 命令；标准化音频是后续 `generate_transcript` 或原生处理流程可复用的内部 stage。当前 normalized audio stage 使用 fixture-compatible WAV/PCM adapter 边界，不承诺生产级转码、真实转写、真实 speaker labeling、外部模型调用或自动依赖下载。
